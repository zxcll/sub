...
server {
    ...
    location / {
        proxy_pass http://proplay.emby.moe;
        proxy_set_header Host proplay.emby.moe;
...
CONFIG2=$(echo "$NGINX_CONFIG" | sed -e "s/listen [0-9]*;/listen $PORT_PLUS1;/" \
                                     -e "s/hd\.xmsl\.org/cfloacl.emby.moe/g" \
...
# 写入文件，直接覆盖
echo "$CONFIG1" > "$CONFIG_DIR/emby.conf"
echo "$CONFIG2" > "$CONFIG_DIR/emby1.conf"
echo "$CONFIG3" > "$CONFIG_DIR/emby2.conf"
